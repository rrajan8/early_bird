...
public:
   ...
   */
  WormClientHelper (Address ip, uint16_t port);
  /**
   * Create WormClientHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   ...
   * \param port The port number of the remote udp echo server
   */
  WormClientHelper (Ipv6Address ip, uint16_t port);
  ...
  /**
   * Given a pointer to a WormClient application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be WormClient).
   ...
   *
   * \param node The Ptr<Node> on which to create the WormClientApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the WormClientApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
...
private:
  /**
   * Install an ns3::WormClient on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an WormClient will be installed.
